...
def _gather_clone_loss(clone, num_clones, regularization_losses):
  ...
  if clone_loss is not None:
    tf.summary.scalar(clone.scope + '/clone_loss', clone_loss) #,name='clone_loss'
  if regularization_loss is not None:
    tf.summary.scalar('regularization_loss', regularization_loss) #name='regularization_loss'
  return sum_loss
...
def deploy(config,
  ...
  with tf.device(config.optimizer_device()):
    ...
    if total_loss is not None:
      ...
      summaries.add(tf.summary.scalar('total_loss', total_loss)) #name='total_loss'
