...
from    model_generic import Model,prelu,enet_pool,enet,enet_unpool
...
class AutopickModel(Model):
    ...
    @staticmethod
    def get_rpn_score(net, scope, nclasses, reuse=False):
        depth = net.shape[-1]._value
        # Feature extractor part is shared for all heads
        with tf.variable_scope(scope, reuse=reuse):
            # reduce to constant channels for weight sharing
            ...
            rpn_score = slim.conv2d(rpn_conv, nclasses, [1, 1], activation_fn=None, scope='rpn_cls_score_2',reuse=reuse)
            return rpn_score
    ...
    def get_batch_image_indices(data, rpn_shape, label):
        ...
        return tf.gather_nd(bidxs, non_ignore)
    ...
    def scores2losses(self,rpn_score, data, reuse=False, scope=''):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            rpn_shape = tf.cast(tf.shape(rpn_score), tf.int64)
            ...
            # ##### SUMMARY ###########
            tf.summary.image('input_image', tf.expand_dims(data['image'][-1, :, :, :1], axis=0))

            # collect foreground scores
            loss = 0.0
            for idx in range(nclasses):
                c      = classes[idx]
                imidxs = self.get_batch_image_indices(data, rpn_shape, c)
                ...
                loss   = loss + tf.reduce_sum(ce)/tf.cast(npart + np.int32(1), tf.float32)
    ...
    def add_loss_and_metrics_train(self,data,logits,end_point_collection):
        loss = self.scores2losses(logits['rpn_score'],data,scope='rpn_head')
        #### ADD LOSS ##########
